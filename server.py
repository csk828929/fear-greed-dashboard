...
def fetch_crypto_fear_greed() -> dict:
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            # Add BTC price factors (Kraken → Binance → CoinGecko)
            try:
                prices = volumes = []
                # Kraken public API
                kr = requests.get("https://api.kraken.com/0/public/OHLC?pair=XBTUSD&interval=1440", timeout=10)
                if kr.status_code == 200:
                    kd = kr.json()
                    ohlc = (kd.get("result", {}).get("XXBTZUSD") or [])[-60:]
                    prices = [float(k[4]) for k in ohlc]
                    volumes = [float(k[6]) for k in ohlc]
                if len(prices) < 20:
                    # Binance fallback
                    br = requests.get("https://api.binance.com/api/v3/klines?symbol=BTCUSDT&interval=1d&limit=60", timeout=10)
